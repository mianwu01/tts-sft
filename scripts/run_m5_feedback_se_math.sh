...
#   loop-0 = the formal verifier-free run's loop-0 population, reused VERBATIM (no regeneration)
...
for ds in aime hmmt; do
  RUN=tts_sft_se_m5fb_${ds}_node2
  ODIR=outputs/node2_math_m5fb_se_${ds}
  CK=external/squeeze-evolve/$ODIR/checkpoints
  SRC=outputs/node1_se_loop5_32k_temp1_${ds}_non_saturated/se.jsonl.checkpoints/tts_sft_se_loop5_32k_temp1_${ds}_non_saturated_node1_loop0.json
...
json.dump({
  "run_name": f"tts_sft_se_m5fb_{ds}_node2", "arm": "M5_feedback_se_math",
  "loop0": "REUSED from the formal verifier-free run (NOT regenerated)", "loop0_source_checkpoint": src,
  "starts_at_loop": 1, "loops_total": 5, "loops_generated": [1, 2, 3, 4],
  "config": f"configs/squeeze_evolve_m5fb_{ds}_node2.yaml",
  ...
  "paired_reference": f"outputs/node1_se_loop5_32k_temp1_{ds}_non_saturated (same loop-0, plain aggregate)",
  "no_sft": True, "update": "replace", "strip_think": False,
...
PY
  ...
  python scripts/run_squeeze_evolve.py \
    ...
    --config configs/squeeze_evolve_m5fb_${ds}_node2.yaml \
